...
public class World
{
	...
	private int light(int x, int y, int currentX, int currentY, double r)
	{
		...
		// ***** <YOUR CODE GOES HERE> *****
		if (currentX < 0 || currentX >= width || currentY < 0 || currentY >= height || tiles[currentX][currentY].getLit())
			return 0;
		int deltaX = currentX - x, deltaY = currentY - y;
		...
		for (int i = 0; i < 4; i++) {
			int dx = (i - 1) % 2;
			int dy = (i - 2) % 2;
			result += light(x, y, currentX + dx, currentY + dy, r);
		}
